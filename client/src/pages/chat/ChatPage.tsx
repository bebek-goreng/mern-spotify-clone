...
const ChatPage = () => {
  ...
  return (
    <main className="h-full rounded-lg bg-gradient-to-b from-zinc-800 to bg-zinc-900 overflow-hidden">
      <Topbar />
              ...
              <ScrollArea className="h-[calc(100vh-340px)]">
                <div className="p-4 space-y-4">
                  {messages.map((message, index) => (
                    <div key={`${message._id}-${index}`} className={`flex items-start gap-3 ${message.senderId === user?.id ? "flex-row-reverse" : ""}`}>
